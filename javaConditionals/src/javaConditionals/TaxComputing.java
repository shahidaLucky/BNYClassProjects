...
public class TaxComputing {
	...
	public static void main(String[] args) {
		...
		while (!(filingStatus >= 1 && filingStatus <= 4)) {
			...
			/*
			 * try-catch block to handle input that is not an integer non-integer throws
			 * InputMismatchException catch block prints error message, resets Scanner
			 * token, and continues loop
			 */
			try {
		...
		/*
		 * while loop to repeat income prompt until a positive number is entered income
		 * was initialized to 0
		 */
		while (income <= 0) {
			System.out.println("Please enter your taxable income:");
			/*
			 * try-catch block to handle input that is not a double non-double throws
			 * InputMismatchException catch block prints error message, resets Scanner
			 * token, and continues loop
			 */
			try {
